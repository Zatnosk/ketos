...
fn webfinger(
    actors_ref: &Arc<Mutex<HashMap<String, Actor>>>,
) -> impl Fn(Vec<(String, String)>) -> Result<String, warp::reject::Rejection> + Clone {
...
fn actor_endpoint(
    actors_ref: &Arc<Mutex<HashMap<String, Actor>>>,
) -> impl Fn(String) -> Result<String, warp::reject::Rejection> + Clone {
    let actors_ref = Arc::clone(actors_ref);
    move |name: String| {
        let actors = match actors_ref.lock() {
            Ok(actors) => actors,
...
fn main() {
    ...
    actors
        .lock()
        .unwrap()
        .insert(String::from("zatnosk"), zatnosk);

    let webfinger = path!(".well-known" / "webfinger")
        .and(warp::query::query())
        .and_then(webfinger(&actors))
